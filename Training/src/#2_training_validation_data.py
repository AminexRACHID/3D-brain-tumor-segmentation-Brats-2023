...
OUT_TRAIN_NNUNET_IMAGES = '../data/training/SAD_Unet_raw/Dataset001_BRATS/imagesTr/'
OUT_TRAIN_NNUNET_LABELS = '../data/training/SAD_Unet_raw/Dataset001_BRATS/labelsTr/'
OUT_TEST_NNUNET_IMAGES = '../data/test/SAD_Unet_raw/imagesTs/'
OUT_TEST_NNUNET_LABELS = '../data/test/SAD_Unet_raw/labelsTs/'
...
def process_case(case, case_num, out_img_dir, out_label_dir):
    ...
    imgs = {'t1n': '*t1.nii.gz', 't1c': '*t1ce.nii.gz',
            't2w': '*t2.nii.gz', 'flair': '*flair.nii.gz'}
    
    for i, (code, fname) in enumerate(imgs.items()):
        print(case_path, fname)
        img = nib.load(glob.glob(case_path + '/' + fname)[0])
        nib.save(img, os.path.join(out_img_dir, f'BRATS_{case_num:04d}_000{i}.nii'))
        print(f"Saved {fname} image as BRATS_{case_num:04d}_000{i}.nii")

    # Load and save label
    seg = nib.load(glob.glob(case_path + '/' + '*seg.nii.gz')[0])
    nib.save(seg, os.path.join(out_label_dir, f'BRATS_{case_num:04d}.nii'))
    print(f"Saved *seg.nii mask as BRATS_{case_num:04d}.nii")
